(function() {
	...
	if ($("#youtube-widget").length) {		
		com.sb.youtube_widget = {};
		com.sb.youtube_widget.$youtube_widget = $("#youtube-widget");		
		com.sb.youtube_widget.username = com.sb.query_string.username;		
		com.sb.youtube_widget.image = "../img/youtube-no-image.png";

		if (typeof(com.sb.youtube_widget.username) === 'undefined' || com.sb.youtube_widget.username === "") {
			com.sb.youtube_widget.$youtube_widget.html("<ul><li>Please add a username</li></ul>");
		} else {
			// Get user image			
			var url = "https://gdata.youtube.com/feeds/api/users/" + com.sb.youtube_widget.username;
			$.ajax({
				url: url,
				type: "GET",
				dataType: "text",
				...
				if (xml.find("entry > thumbnail").length) {									
					com.sb.youtube_widget.image = xml.find("entry > thumbnail").attr("url");
				} else {
					com.sb.youtube_widget.image = xml.find("entry > media\\:thumbnail").attr("url");
				}					
			});
		
			// Try to get live data first
			var url = "https://gdata.youtube.com/feeds/api/users/" + com.sb.youtube_widget.username + "/live/events?v=2&status=active&inline=true";
			$.ajax({
				url: url,
				type: "GET",
				dataType: "text",
				...
				com.sb.youtube_widget.title = xml.find("feed > entry:first > title").text();
				
				if (xml.find("feed > entry > content > entry > group > videoid").length) {									
					com.sb.youtube_widget.video_id = xml.find("feed > entry > content > entry > group > videoid").text();
				} else {
					com.sb.youtube_widget.video_id = xml.find("feed > entry > content > entry > media\\:group > yt\\:videoid").text();
				}
				
				if (xml.find("feed > entry > content > entry > statistics").length) {									
					com.sb.youtube_widget.viewers = xml.find("feed > entry > content > entry > statistics").attr("currentViewers");
				} else {
					com.sb.youtube_widget.viewers = xml.find("feed > entry > content > entry > yt\\:statistics").attr("currentViewers");
				}
				
				if (xml.find("entry").length) {
					// Live
					com.sb.youtube_widget.$youtube_widget.html("<ul class='youtube-widget-list'><li class='user-name'><a href='" + (com.sb.youtube_widget.video_id ? "http://www.youtube.com/watch?v=" + com.sb.youtube_widget.video_id : "http://www.youtube.com/" + com.sb.youtube_widget.username) + "' target='_blank'><img src='" + com.sb.youtube_widget.image + "' width='44' height='44' alt='" + com.sb.youtube_widget.username + " channel logo' class='icon'>" + com.sb.youtube_widget.username + "</a></li><li class='live'><b>LIVE</b> <span class='online'></span> " + com.sb.youtube_widget.title  + "</li><li class='viewers'><img src='../img/eye-gray.png' width='14' height='14' alt='' class='icon-eye' /> " + com.sb.youtube_widget.viewers + "</li></ul>");
				} else {
					// Offline
					com.sb.youtube_widget.$youtube_widget.html("<ul class='youtube-widget-list'><li class='user-name'><a href='http://youtube.com/" + com.sb.youtube_widget.username + "' target='_blank'><img src='" + com.sb.youtube_widget.image + "' width='44' height='44' alt='" + com.sb.youtube_widget.username + " channel logo' class='icon'>" + com.sb.youtube_widget.username + "</a></li><li class='live'><b>Offline</b></li></ul>");
				}					
			});			
		}
	}
